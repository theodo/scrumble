...
.controller 'StatsCtrl', ['$scope', 'DailyReportPing', 'Project', ($scope, DailyReportPing, Project) ->
  ...
  DailyReportPing.find().then (_pings_) ->
    ...
    Project.query(
      ...
      projectsByOrganization = _.groupBy(projects, (project) -> project.organization?.name)
      for organization, projects of projectsByOrganization
        for project in projects
          project.pings = _.groupBy(project.DailyReportPings, (ping) -> moment(ping.createdAt).format('YYYY-MM-DD'))
      $scope.projectsByOrganization = projectsByOrganization
    # Organization.query(
    #   filter:
    #     include:
