...
install:
	...
	sudo chown -R ${whoami}:${group} api/node_modules && \
	docker-compose --file docker-compose.build.yml run --rm appbuilder npm install --quiet --unsafe-perm && \
	sudo chown -R ${whoami}:${group} client/node_modules && \
	sudo chown -R ${whoami}:${group} client/bower_components
...
migration-create:
	...
	./node_modules/db-migrate/bin/db-migrate create --config migrations/database.json $(name)\
	 && sudo chown -R ${whoami}:${group} api/migrations
migration-up:
	eval "$$(docker-machine env -u)" && \
	docker-compose --file docker-compose.dev.yml run --rm api \
...
npm-install:
	...
	sudo chown ${whoami}:${group} api/package.json &&\
	sudo chown -R ${whoami}:${group} api/node_modules
...
client-npm-install:
	...
	sudo chown -R ${whoami}:${group} ./client/node_modules && \
	sudo chown -R ${whoami}:${group} ./client/package.json
